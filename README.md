...
すでに`schema.sql`を実行済みのプロジェクトでは、課題の個別非表示機能を追加するため、`supabase/migrations/002_assignment_preferences.sql`だけをSQL Editorで追加実行します。
...
課題一覧の「表示しない」を押すと、その課題は通常の一覧と集計から除外されます。「非表示にした課題」フィルターから再表示できます。学内情報ハブから、公式のシラバス、履修案内、試験時間割、キャンパス別の鉄道運行情報へ直接移動できます。
...
- `assignment_preferences`: ユーザーが非表示にしたCanvas課題のID
